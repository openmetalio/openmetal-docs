...
> some Windows distributions worked more reliably with slightly older virtio-win drivers
> (for example, Windows 10 Enterprise ran more smoothly on 0.1.215-2 than on 0.1.217-2). 
> However, at the time of writing, the latest release virtio-win-0.1.271-1 has worked 
> without issues in our tests.
...
Let’s make sure the system supports virtualization and acceleration by running `grep -Eoc '(vmx|svm)' /proc/cpuinfo` and `kvm-ok`.
...
so we used win2k22 instead. The installation completed successfully without issues.

### Start the Instance

Since our `libvirt-qemu` user does not have permissions to access our files in `/home/ubuntu`, we'll move them to a different path:
```
...
Congratulations! You have successfully built and configured a Windows Server 2025
...
Our hard drive was automatically expanded to the `gen2.giant` flavor defined size.
